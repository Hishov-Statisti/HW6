...
```{r}
-.5*t(grad_f.Q6(10,10))%*%(grad_f.Q6(10,10))
...
while (TRUE) {
  ...
  gamma.j <- gamma.j.Q6(alpha,beta,gamma = 1,x.j,y.j)
...
```{r}
  while (f.next > f.now - .1 * gamma * t(grad_f.Q6(x,y))%*%(grad_f.Q6(x,y))) {
      gamma <- beta * gamma
      #print(c(gamma,f.now,f.next))
    }
