...
```{r,echo=FALSE}
#knitr::include_graphics("2.c.png")
```
...
The Information matrix (-E[hesian]) is not a function of a random variable so N-R algorithm and FS algorithm are same.
...
```{r,echo=FALSE}
#knitr::include_graphics("5.a.png")
```
...
```{r}
gamma.j.Q6 <- function(f.now,f.next,alpha,beta,gamma,x,y) {
    while (TRUE) {
      temp <- f.next > f.now - alpha * gamma * norm(x = grad_f.Q6(x,y),type = "2")
      if(!temp)(break)
      gamma <- beta * gamma
      print(gamma,f.now,f.next)
    }
  return(gamma)
}
```
...
```{r}
t(grad_f.Q6(5,5))%*%(grad_f.Q6(5,5))
norm(x = grad_f.Q6(5,5),type = "2")^2
```


```{r}

x.j <- y.j <- 10  # first guess
...
while (TRUE) {
  ...
  #f.j <- f.Q6(x.j,y.j) # f.now
  x_y.j_1 <- x_y.j - gamma.j * grad_f.Q6(x.j,y.j) # x,y next value
  #f.j_1 <- f.Q6(x_y.j_1[1],x_y.j_1[2]) # f.next
  
  #gamma.j <- gamma.j.Q6(f.j,f.j_1,alpha,beta,gamma = 1,x.j,y.j)
  
  #x_y.j_1 <- x_y.j - gamma.j *  grad_f.Q6(x.j,y.j)
  
  x.j <- x_y.j_1[1]
  y.j <- x_y.j_1[2]

  
  if (max(abs(x_y.j_1 - x_y.j)) < eps_for_break) {break}
...
```{r,echo=FALSE}
cat(sprintf("x (Gradient Descent) = %s", x.j),
